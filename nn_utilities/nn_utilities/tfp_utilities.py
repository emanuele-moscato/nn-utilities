...
def plot_uncertainty(
    ...
    if how == 'means':
        ...
        sns.scatterplot(
            x=tf.reshape(
                tf.constant([
                    [x_position.numpy()] * means.shape[0]
                    for x_position in x_interval
                ]),
                x_interval.shape[0] * means.shape[0]
            ).numpy(),
            y=tf.reshape(
                tf.transpose(means), means.shape[0] * means.shape[1]
            ),
            color=sns.color_palette()[0],
            label='Means',
            alpha=.5
        ...
        sample_means = tf.reduce_mean(
            tf.reshape(
                tf.transpose(samples, perm=[2, 0, 1]),
                (samples.shape[2], samples.shape[0] * samples.shape[1])
            ),
            axis=1
        )
        ...
        sns.scatterplot(
            x=tf.reshape(
                tf.constant([
                    [x_position.numpy()] * (samples.shape[0] * samples.shape[1])
                    for x_position in x_interval
                ]),
                x_interval.shape[0] * samples.shape[0] * samples.shape[1]
            ).numpy(),
            y=tf.reshape(
                tf.transpose(samples, perm=[2, 0, 1]),
